...
class HACNN(nn.Module):
	...
	# Construct Attribute generator block
        ...
        self.classifier_att = nn.Linear(100, 26)
    ...
    def forward(self, x):
        ...
        xa = self.att(x)
        xa_fin= F.avg_pool2d(xa,
                             xa.size()[2:]
                             ).view(xa.size(0), xa.size(1))
        x_att = self.fc_att(xa_fin)
        prelogits_att = self.classifier_att(x_att)	      
        ...
        if not self.training:
            ...
            if self.learn_region:
                ...
                aa = prelogits_att / prelogits_att.norm(p=2, dim=1, keepdim=True)
                return torch.cat([x_global, x_local,aa], 1)
            else:
                return x_global
        ...
        if self.loss == 'softmax':
            if self.learn_region:
                return (prelogits_global, prelogits_local,prelogits_att)
            else:
